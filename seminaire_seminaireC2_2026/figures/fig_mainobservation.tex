...
\only<1,2>{
\node[anchor = north west] at (-15,16) {\footnotesize Verification $(C = C_0 || \cdots || C_{L-1},T)$}; 
}
\only<3->{
\node[anchor = north  west] at (-15,16) {\footnotesize Verification  $(\cipherblock^\cipherlength, T)$}; 
}

\node[anchor = east] at (100,15){}; 
...
\only<1,2>{
    \draw (-0.5,11.5) node[above] {\footnotesize$\ciphertext_{L-1}$};
}
\only<3->{
...
\only<1,2>{
\draw[fill=vert3!20] (23.5,0) rectangle (25.9,6);
