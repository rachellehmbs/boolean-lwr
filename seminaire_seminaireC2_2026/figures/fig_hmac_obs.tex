...
%\only<1>{
%\node[anchor = north west] at (-3,2.5) {\footnotesize $M = M_1 || \cdots || M_{L}$}; 
%
...
%	\path (18+2,0.5) node{\footnotesize $|M|$};
%	
%}
%\only<2->{
%\node[anchor = north  west] at (-3,2.5) {\footnotesize  $M = \cipherblock^\cipherlength$}; 
...
%}
	...
	\end{scope}
		...
		\node [draw,trapezium,trapezium left angle=60,trapezium right angle=0,minimum height=0.7cm,fill=Melon!10,shift={(1.15,0.4)},rotate=-90] 
		(h) {\begin{sideways}${\color{cyan!10}h}$\end{sideways}};
		\draw[-latex] ++(1.5,+4) -- ++(0,-2) -- ++(0.5,0);
		\draw[-latex] ++(-0.2,0.7) --  ++(2.3,0);
		\draw[-latex] ++(3.8,0.7) --  ++(1,0);
		\path (2.95,0.9) node{\footnotesize $g_{\rouge{K}}$};

		\draw[fill=rouge3!20] (0.1,0) rectangle (1.2,2);
	...
	\path (14+7+1+1.8,-3.3) node{\footnotesize $\ver{T}$};
