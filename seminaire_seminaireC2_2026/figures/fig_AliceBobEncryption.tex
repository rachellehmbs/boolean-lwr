...
\begin{tikzpicture}[scale=0.3]
  ...
  \node[below = 1em of alice] (alicekey) {\Huge \color{rouge3} $\key$};
    \node[below = 1em of alicekey] (msgalice) {\Huge \color{vert2} $M$};
  
      \onslide<2,3,4>{
    \node[below = 1em of msgalice] (ctalice) {\Huge ${\color{vert2} C} = \encryptionalgo({\color{rouge3} \key},{\color{vert2} M})$ };
  }
  
  % Bob
  \node (label) at (30,0) {\includegraphics[height=7cm]{figures/canal_bob_dep.png}};
  \node (bob) at (30,-4) {\Huge\bf Bob};
  \node[below = 1em of bob] (bobkey) {\Huge \color{rouge3} $\key$};
      \onslide<4>{
    \node[below = 1em of bobkey] (msgbob){\Huge ${\color{vert2} M} = Dec({\color{rouge3} \key},{\color{vert2} C})$ }; 
  }
  ...
  \draw[->, line width=1mm,color=gris2] (18,-7) -- ++(+3,0) --  node[right,align=center] {\Huge\bf \color{gris2}Modify \\ \huge (Integrity)} ++(0,+5);
  ...
  \onslide<3,4>{
    \node[below = 3em of channel] {\Huge $\longrightarrow \quad$ \color{vert2} $C$ \color{black} $\quad \longrightarrow$};
  } 
