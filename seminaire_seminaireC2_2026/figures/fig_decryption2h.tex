...
\draw[->,>=latex] (2.5,10) node[above] {\small$\associateddata$} -- ++ (0,-1);

\draw[SpongePerm] 
    	(1.5,0) rectangle node { \small $F_{init}$} ++(2,9);
...
\begin{scope}[xshift=\z*5.9cm]
    ...
    \draw (1,9.5) node[above] { \small $C_{\z}$} -- (1,8);
...
\draw[SpongePerm] 
    		(2.3,0) rectangle node {\small $h$} ++(1,9);
...
\begin{scope}[xshift=20.7cm]
    ...
    \draw (1.8,9.5) node[above] {\small $C_{l-1}$} -- (1.8,8);
...
%\draw(0,7) rectangle node{\small $\cipherblock$} ++ (1.8,3);
  ...
  \draw[SpongePerm] 
    	(2.5,0) rectangle node { \small $F_{final}$} ++(2.5,9);
     \draw[->,>=latex] (1,8) -- ++(1.5,0);
          \draw[->,>=latex] (1,2) -- ++(1.5,0);
          
     \draw (5,6) --++ (1.1,0);
    \draw[->,>=latex] (6.1,6) --++ (0,4.3);
   \draw (6.1,10) node[above] {\small $\tagAEAD$};
\end{scope}
